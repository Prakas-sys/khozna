      ...
      <div className="max-w-[1400px] mx-auto px-10 py-12">
        ...
        >
          <div>
            <div className="flex items-center gap-4 mb-3">
              <h2 className="text-3xl font-extrabold text-[#0F172A] tracking-tight">Property Moderation</h2>
              <span className="px-3 py-1 bg-[#2563EB]/10 text-[#2563EB] text-[10px] font-bold uppercase tracking-wider rounded-full">
                {properties.filter(p => p.status === 'pending').length} Pending Review
              </span>
            </div>
            <p className="text-[#64748B] text-sm font-medium">Validating asset integrity and market compliance across the ecosystem.</p>
          </div>
          
          <div className="flex items-center gap-3">
             <button onClick={fetchProperties} className="h-11 px-6 bg-white border border-[#E2E8F0] rounded-2xl hover:bg-[#F8FAFC] flex items-center gap-2.5 font-bold transition-all text-xs text-[#64748B] group">
               <RefreshCcw size={16} className="group-hover:rotate-180 transition-transform duration-700" /> 
               Refresh
             </button>
             <button className="w-11 h-11 flex items-center justify-center bg-white border border-[#E2E8F0] rounded-2xl text-[#64748B] hover:bg-[#F8FAFC] transition-all">
               <Filter size={18} />
             </button>
          </div>
        </motion.div>
        ...
        {loading ? (
          <div className="flex flex-col justify-center items-center py-40 gap-4">
            ...
            <p className="text-[#94A3B8] text-xs font-bold uppercase tracking-widest">Scanning inventory</p>
          </div>
        ) : properties.length === 0 ? (
            ...
            <AnimatePresence mode="popLayout">
              {properties.map((p, idx) => (
                <motion.div 
                  ...
                  className="card-platinum rounded-[2.5rem] p-6 flex flex-col md:flex-row gap-8 group"
                >
                  <div className="h-52 w-full md:w-52 flex-shrink-0 rounded-[2rem] overflow-hidden bg-[#F8FAFC] border border-[#E2E8F0] relative group/img">
                  ...
                  <div className="flex-1 flex flex-col justify-between py-1">
                    <div>
                      <div className="flex items-start justify-between mb-3">
                        <h3 className="text-xl font-extrabold text-[#0F172A] leading-tight line-clamp-2 pr-4">{p.title}</h3>
                        <button className="text-[#94A3B8] hover:text-[#2563EB] transition-colors"><ExternalLink size={18} /></button>
                      </div>
                      
                      <div className="space-y-2.5">
                        <div className="flex items-center gap-2.5 text-[#64748B] font-bold text-[10px] uppercase tracking-wider">
                          <MapPin size={14} className="text-[#2563EB]/40" />
                          <span>{p.area_name}, {p.city}</span>
                        </div>
                        <div className="flex items-center gap-2.5 text-[#64748B] font-bold text-[10px] uppercase tracking-wider">
                          <Building2 size={14} className="text-[#2563EB]/40" />
                          <span>Listed by: <span className="text-[#0F172A]">{p.profiles?.full_name || 'Anonymous User'}</span></span>
                        </div>
                      </div>
                      
                      <div className="mt-6 flex items-center gap-1.5">
                        <Tag size={12} className="text-[#2563EB]" />
                        <span className="text-2xl font-extrabold text-[#2563EB] tracking-tight">रू {p.price.toLocaleString()}</span>
                        <span className="text-[10px] font-bold text-[#94A3B8] uppercase tracking-wider ml-1">Total Value</span>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-4 mt-8">
                      ...
                      <button 
                        ...
                        disabled={processingId === p.id}
                        className="flex-1 h-12 bg-white border border-[#E2E8F0] text-[#64748B] font-bold rounded-2xl flex items-center justify-center gap-2 hover:bg-red-50 hover:text-red-500 hover:border-red-100 transition-all disabled:opacity-50 active:scale-95 text-xs"
                      >
