...
CREATE POLICY "Users can manage own saves" ON public.saved_properties FOR ALL USING (auth.uid() = user_id);

-- 4. KYC: Private per user
CREATE POLICY "Users can view/submit own KYC" ON public.kyc_verifications FOR ALL USING (auth.uid() = user_id);

-- 5. Notifications: Private per user
CREATE POLICY "Users can view/manage own notifications" ON public.notifications FOR ALL USING (auth.uid() = user_id);
