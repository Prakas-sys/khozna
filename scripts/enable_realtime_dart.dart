...
void main() async {
  ...
  try {
    final result = await Process.run('psql', [
      '--url', 'postgresql://postgres.qjpeablwokiuhfaopdbi:Khozna%40Success%23@aws-0-ap-southeast-1.pooler.supabase.com:6543/postgres',
      '-c', sql
    ]);
    ...
    if (result.exitCode == 0) {
      ...
      print('--- ⚠️ Please paste the SQL above into your Supabase Dashboard SQL Editor for the FINAL fix. ---');
    }
  } catch (e) {
    print('--- ⚠️ psql not found. Manual step required! ---');
    print('--- 💡 Action: Copy the SQL block above and run it in your Supabase SQL Editor. ---');
  }
}
